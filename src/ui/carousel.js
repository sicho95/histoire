...
function scrollByCard(delta) {
  ...
  if (!card) return;
  container.scrollBy({ left: (card.offsetWidth + 24) * delta, behavior: 'smooth' });
}
...
export function renderHome() {
  ...
  document.getElementById('carousel-prev').onclick = () => scrollByCard(-1);
  document.getElementById('carousel-next').onclick = () => scrollByCard(1);
}
