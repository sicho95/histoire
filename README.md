...
La génération utilise l'offre gratuite Groq avec `openai/gpt-oss-120b` et des sorties JSON structurées. La clé est saisie dans l'espace parents et reste en session par défaut. Elle n'est jamais ajoutée au dépôt ou aux exports. Pour respecter le plafond gratuit de 8 000 tokens par minute, une histoire complète est produite en une seule requête compacte, en raisonnement léger, sans réécriture monolithique ni relance automatique. Si Groq fournit tout de même un JSON récupérable malgré un oubli technique, l’application le normalise localement. Chaque route est ensuite mesurée avec le débit observé des narrateurs et le brouillon n’est enregistré que si sa durée et son arc émotionnel atteignent les seuils demandés ; sinon les choix du formulaire restent saisis pour un nouvel essai après le rétablissement du quota.
