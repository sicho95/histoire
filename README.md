...
À chaque embranchement éditorial, la narration pose la question puis lit distinctement les choix numéro 1, 2 et éventuellement 3. Les illustrations signature restent entièrement visibles, sans pastille superposée. Pour une branche générée `2-5 ans`, les repères deviennent de grands carrés unis bleu, vert et rouge, annoncés avec le libellé exact du choix ; les `5-9 ans` montrent seulement les grands chiffres 1, 2 et 3 dans les mêmes couleurs. Pour une autre idée, le micro attend quelques secondes le début de la parole, s’arrête automatiquement après le silence qui suit la phrase et n’appelle jamais le LLM si rien n’a été dit. Sur iPhone, la sortie audio repasse ensuite explicitement en mode haut-parleur de lecture.

Voir [stories/README.md](stories/README.md) et [audio/README.md](audio/README.md).

## Création, voix et export

La génération utilise l'offre gratuite Groq avec `openai/gpt-oss-120b` et des sorties JSON structurées. La clé est saisie dans l'espace parents et reste en session par défaut. Elle n'est jamais ajoutée au dépôt ou aux exports. La première passe produit la structure complète. L’application mesure ensuite chaque route et calcule, selon le rôle de chaque scène, le nombre minimal de mots nécessaire pour atteindre la durée demandée. Après le délai `x-ratelimit-reset-tokens` fourni par Groq — ou une minute si ce renseignement est indisponible — elle demande des remplacements complets avec des minima exécutables, par lots compatibles avec le plafond gratuit. Une réponse qui reste sous le minimum est refusée et retentée au passage suivant. La dernière passe renforce la cohérence et les émotions sans jamais raccourcir une scène. Aucun brouillon n’est enregistré avant la validation finale ; les choix du formulaire restent saisis en cas d’échec.

L’ambiance choisie définit sa propre progression émotionnelle : rire, mystère, musique, apaisement, sauvetage, émotion forte ou frisson. Le texte additionnel est un modificateur prioritaire : il peut renforcer cette direction ou en ajouter une autre. « Tragique » et « faire pleurer d’émotion » imposent un lien, une rupture, un souvenir, un geste coûteux et des retrouvailles méritées. « Plus de peur », « plus de frisson » ou « chair de poule » imposent plusieurs montées de suspense et au moins deux pics vocaux, toujours sans violence ni angoisse durable ; pour les 2–5 ans, la peur reste plafonnée à un bruit ou une ombre vite expliqués.
