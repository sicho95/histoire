...
La génération utilise l'offre gratuite Groq avec `openai/gpt-oss-120b` et des sorties JSON structurées. La clé est saisie dans l'espace parents et reste en session par défaut. Elle n'est jamais ajoutée au dépôt ou aux exports. La première passe produit une histoire complète compacte. L’application lit ensuite le délai `x-ratelimit-reset-tokens` fourni par Groq — ou attend une minute si ce renseignement est indisponible — puis lance une passe éditoriale qui renvoie uniquement les scènes corrigées. Une histoire de 20 minutes est relue en plusieurs parties afin que chaque requête reste sous le plafond gratuit de 8 000 tokens par minute. Une correction supplémentaire est effectuée si la durée ou l’arc émotionnel échoue encore au contrôle local. Aucun brouillon n’est enregistré avant la validation finale ; les choix du formulaire restent saisis en cas d’échec.
