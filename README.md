...
La génération utilise l'offre gratuite Groq avec `openai/gpt-oss-120b` et des sorties JSON structurées. La clé est saisie dans l'espace parents et reste en session par défaut. Elle n'est jamais ajoutée au dépôt ou aux exports. Si Groq oublie exceptionnellement une propriété technique, l'application récupère la réponse quand elle est disponible ou effectue une seule relance de contrôle ; un brouillon partiel ne remplace jamais une histoire valide. Chaque route est mesurée avec le débit observé des narrateurs : une histoire trop courte est automatiquement soumise à une seconde passe éditoriale et n’est pas enregistrée si sa route la plus courte reste sous 90 % de la durée choisie. Une idée libre déclenche aussi cette passe afin que l’émotion ou le thème demandé structure réellement le récit.
