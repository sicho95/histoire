...
- Une histoire complète doit tenir dans une seule requête sous le plafond gratuit Groq de `8 000 TPM` : raisonnement léger, budget de sortie borné, graphe compact à décisions communes et conséquences courtes. Aucune seconde réécriture complète ni relance automatique ne doit doubler le quota. Un JSON rejeté mais récupérable est normalisé localement. Après un échec, les choix du formulaire restent saisis et aucun identifiant de compte ni détail technique n’est montré à l’enfant.
