...
- Une première requête compacte crée l’histoire sous le plafond gratuit Groq de `8 000 TPM`. Les passes suivantes attendent `x-ratelimit-reset-tokens` avec une marge, ou une minute depuis le début de la requête précédente si cet en-tête manque. Elles allongent d’abord les seules scènes mesurées comme trop courtes, par lots, puis effectuent une passe éditoriale ciblée sur les décisions et les fins. La réémission monolithique de toute l’histoire reste interdite.
- La durée d’une création est mesurée sur chaque route réellement jouable, questions et choix lus compris, avec le débit observé des voix du projet (`120 mots/min` pour les `2-5 ans`, `145 mots/min` pour les `5-9 ans`). Une histoire visant dix minutes doit durer au moins neuf minutes sur sa route la plus courte. Le contrôle ne se contente plus de constater l’échec à la fin : il calcule des minima de mots par scène (décision, conséquence ou fin), les impose dans les demandes d’allongement et refuse localement tout remplacement encore trop court. La passe émotionnelle finale ne peut jamais raccourcir le récit. Un résultat restant hors cible n’est pas enregistré et les choix du formulaire sont conservés.
